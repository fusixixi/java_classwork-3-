...
public class StudentController extends ToolController {
    ...
    @FXML
    private TableColumn<Map, String> passwordColumn;//学生信息表 密码列
    @FXML
    ...
    @FXML
    private TextField passwordField;  //密码输入域
    @FXML
    private CheckBox showPasswordCheckBox;  //密码可见性切换

    // 验证相关字段
    @FXML
    private Label numErrorLabel;  // 学号错误提示
    @FXML
    ...
    private boolean isNewStudent = false;  //是否新增学生标识
    ...
    private void initValidator() {
        ...
        if (numField != null && numErrorLabel != null) {
            validator.addRealTimeValidator(numField, numErrorLabel,
                InputValidator::isNotEmpty, InputValidator.MSG_STUDENT_ID_EMPTY);
        }

        if (passwordField != null && passwordErrorLabel != null) {
            validator.addRealTimeValidator(passwordField, passwordErrorLabel,
                value -> !isNewStudent || InputValidator.isNotEmpty(value),
    ...
    private boolean validateForm() {
        ...
        // 验证密码（新增时必填）
        if (isNewStudent && passwordField != null && passwordErrorLabel != null) {
    ...
    protected void onSaveButtonClick() {
        ...
        // 新增学生时包含密码
        if (isNewStudent && passwordField.getText() != null && !passwordField.getText().isEmpty()) {
...
//        DataResponse res =HttpRequestUtil.uploadFile("/api/base/uploadPhoto",file.getPath(),"photo/" + personId + ".jpg");  //上传保存到服务器目录/photo/主键PersonId.jpg
    ...
    protected void onResetPwdButtonClick() {
        ...
        if (num == null || num.isEmpty()) {
            MessageDialog.showDialog("请先选择或输入学生学号");
            return;
        }
        int ret = MessageDialog.choiceDialog("确认要将学生 " + num + " 的密码重置为默认密码 123456 吗？");
        if (ret != MessageDialog.CHOICE_YES) {
            return;
        }
        DataRequest req = new DataRequest();
