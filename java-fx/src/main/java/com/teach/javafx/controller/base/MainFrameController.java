...
public class MainFrameController {
    ...
    public void initMenuTree(List<Map> mList) {
        ...
        for(i = 0; i < mList.size();i++) {
            m = mList.get(i);
            sList = (List<Map>)m.get("sList");
            // 安全处理 isLeft 字段，避免类型转换异常
            Object isLeftObj = m.get("isLeft");
            if (isLeftObj instanceof Integer) {
                isLeft = (Integer) isLeftObj;
            } else if (isLeftObj instanceof Number) {
                isLeft = ((Number) isLeftObj).intValue();
            } else {
                isLeft = 0;
            }
        ...
        menuTree.addEventFilter(MouseEvent.MOUSE_CLICKED, new EventHandler<MouseEvent>(){
            public void handle(MouseEvent event){
                ...
                if(name == null || name.length() == 0)
                    return ;
                // 跳过父菜单（有子节点的菜单或以 Modules 结尾的菜单）
                if (!treeItem.getChildren().isEmpty() || name.endsWith("Modules")) {
                    return;
                }
    ...
    public  void changeContent(String name, String title) {
        ...
        if(tab == null) {
            ...
            if(scene == null) {
                String fxmlPath = name + ".fxml";
                java.net.URL resourceUrl = MainApplication.class.getResource(fxmlPath);
                System.out.println("DEBUG: Loading FXML '" + fxmlPath + "' from URL: " + resourceUrl);
                if (resourceUrl == null) {
